...
exports.getVolumes = async(scan) => {
    ...
    try {
        ...
        const match = html.match(/class=('|")chapter[\d\w]+?('|")/gi)[0].replace(/class=('|")(chapter[\d\w]+?)('|")/gi, '$2');

        const list = document.getElementsByClassName(match)[0];
        ...
        for(const l of link){
            if (!l.getAttribute('class').includes('volume-')) break;
